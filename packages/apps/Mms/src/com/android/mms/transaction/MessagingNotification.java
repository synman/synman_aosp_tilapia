...
public class MessagingNotification {
    ...
    private static void updateNotification(
        ...
        if (messageCount == 1) {
            ...
            CharSequence txt = mostRecentNotification.formatBigMessage(context);
            noti.setContentText(txt);

            // TextToSpeech - SMS
            SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(context);
            
            if (isTtsReady && sp.getBoolean("pref_key_sms_notification_tts", false)) {
                final HashMap<String, String> params = new HashMap<String, String>();
                params.put(Engine.KEY_PARAM_VOLUME, ".75");
                params.put(Engine.KEY_PARAM_UTTERANCE_ID, "Message from " + title + ". " + txt.toString());
            ...
            if (uniqueThreadCount == 1) {
                ...
                for (int i = len - 1; i >= 0; i--) {
                    ...
                    CharSequence txt = info.formatBigMessage(context);
                    buf.append(txt);
                    
                    // TextToSpeech - SMS
                    SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(context);
                    
                    if (isTtsReady && i == 0 && sp.getBoolean("pref_key_sms_notification_tts", false)) {
                        final HashMap<String, String> params = new HashMap<String, String>();
                        params.put(Engine.KEY_PARAM_VOLUME, ".75");
                        params.put(Engine.KEY_PARAM_UTTERANCE_ID, "Message From " + title + ". " + txt.toString());
